...
output_folder_path = r'C:/Users/15156/Desktop/motif_JASMS_paper/MotifQuest/New folder'
t_val = [0]
min_motif_len = 2
motif_instances = 2
flank_size = 0
in_file = "Crustacean_NP_DB.fasta"
...
subprocess.call(command_str_alignment, cwd=working_dir)
...
command_str_distance_matrix = f'clustalo -i "{in_file}" -o "{out_file}" --distmat-out="{distance_matrix_file}" --force --full -v'
...
for t in t_val:
    ...
    for fuck, ugh in t_value_data.items():
        if 'consensus_sequences' in ugh and isinstance(ugh['consensus_sequences'], dict):
            consensus_sequences = ugh['consensus_sequences']
    ...
    with open(output_folder_path + '\\partial_motif' + str(t) + 'np.csv', 'w', newline='') as file:
        writer = csv.writer(file)
    ...
    # #full motifs and their matching sequence 
    # full_motif_matches = {}
    
    # for motif in full_motif_storage:
    #     matching_sequences = unaligned_sequences_df[unaligned_sequences_df['Sequence'].str.contains(motif)]['Sequence'].tolist()
    #     full_motif_matches[motif] = matching_sequences
    full_motif_matches = {}
    ...
    full_motif_df.to_csv(output_folder_path + '\\full_motif_report' + str(t) +'.csv')
    partial_motif_df.to_csv(output_folder_path + '\\partial_motif_report' + str(t) + '.csv')
